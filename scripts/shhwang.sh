...
CURRENT_DDAY=$(grep -o '"message": "D-[0-9]*"' data/shhwang.json | grep -o '[0-9]*')
...
if [ "$DAYS_LEFT" != "$CURRENT_DDAY" ]; then
	...
	sed -i "s/\"message\": \"D-[0-9]*\"/\"message\": \"D-$DAYS_LEFT\"/" data/shhwang.json
	git config --local user.email "github-actions[bot]@users.noreply.github.com"
	git config --local user.name "github-actions[bot]"
	git add data/shhwang.json
	git commit -m ":sparkles: update: d-day counter (shhwang)"
	git push
else
